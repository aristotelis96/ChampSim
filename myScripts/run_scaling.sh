...
task(){
	...
	(./bin/scalingExec/${MODE}/${BINARY} -warmup_instructions ${N_WARM} -simulation_instructions ${N_SIM} -perfect_H2P -perfect_H2P_file=${H2P_LOG_DIR}/${TRACE}.txt ${OPTION} -traces ${TRACE_DIR}/${TRACE}) &>  ./results/scalingResults/InstrWindowReset/${BINARY}/${TRACE}._.${BINARY}.txt 
}

for BINARY in `ls ./bin/scalingExec/${MODE} | grep "x08"`; do
	mkdir -p ./results/scalingResults/InstrWindowReset/${BINARY}
	for TRACE in `cat ./myScripts/H2PTraces.txt | grep "641.leela_s-862\|648\|657"`; do		
