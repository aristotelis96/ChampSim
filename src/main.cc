...
// gzip file to collect all branches
bool collect_all = false;
string collect_all_file;
ostream allBranchesFile(NULL);
ofstream branchesFile;
...
long long accuracy, occurrences, misspredictions, reset_window;
...
void print_branch_stats()
{
	...
	cout << "BRANCH_JMP_DIRECT_UNCOND: " << ooo_cpu[i].total_branch_types[1] << " " << (100.0*ooo_cpu[i].total_branch_types[1])/(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) << "%" << endl;
	cout << "BRANCH_JMP_INDIRECT_UNCOND: " << ooo_cpu[i].total_branch_types[2] << " " << (100.0*ooo_cpu[i].total_branch_types[2])/(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) << "%" << endl;
	cout << "BRANCH_JMP_DIRECT_COND: " << ooo_cpu[i].total_branch_types[3] << " " << (100.0*ooo_cpu[i].total_branch_types[3])/(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) << "%" << endl;
	cout << "BRANCH_JMP_INDIRECT_COND: " << ooo_cpu[i].total_branch_types[4] << " " << (100.0*ooo_cpu[i].total_branch_types[4])/(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) << "%" << endl;
	cout << "BRANCH_CALL_DIRECT_UNCOND: " << ooo_cpu[i].total_branch_types[5] << " " << (100.0*ooo_cpu[i].total_branch_types[5])/(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) << "%" << endl;
	cout << "BRANCH_CALL_INDIRECT_UNCOND: " << ooo_cpu[i].total_branch_types[6] << " " << (100.0*ooo_cpu[i].total_branch_types[6])/(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) << "%" << endl;
	cout << "BRANCH_CALL_DIRECT_COND: " << ooo_cpu[i].total_branch_types[7] << " " << (100.0*ooo_cpu[i].total_branch_types[7])/(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) << "%" << endl;
    cout << "BRANCH_CALL_INDIRECT_COND: " << ooo_cpu[i].total_branch_types[8] << " " << (100.0*ooo_cpu[i].total_branch_types[8])/(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) << "%" << endl;
    cout << "BRANCH_RETURN_UNCOND: " << ooo_cpu[i].total_branch_types[9] << " " << (100.0*ooo_cpu[i].total_branch_types[9])/(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) << "%" << endl;
    cout << "BRANCH_RETURN_COND: " << ooo_cpu[i].total_branch_types[10] << " " << (100.0*ooo_cpu[i].total_branch_types[10])/(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) << "%" << endl;
    cout << "BRANCH_OTHER: " << ooo_cpu[i].total_branch_types[11] << " " << (100.0*ooo_cpu[i].total_branch_types[11])/(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) << "%" << endl << endl;    
    }
...
int main(int argc, char** argv)
{
    ...
    while (1) {
        ...
        c = getopt_long_only(argc, argv, "wihIsbpHaomrPCuMRfBFeN", long_options, &option_index);
	...
	//branch_history->printH2PStats();
    }
    // Collect all branches at gzip file
    if (collect_all){
        if(collect_all_file==""){
            printf("GZIP COLLECT ALL BRANCHES FILE NOT EXIST\n");
            assert(false);
        }
        branchesFile.open(collect_all_file, ios_base::out | ios_base::binary);
        outbuf.push(boost::iostreams::gzip_compressor());
        outbuf.push(branchesFile);
        //Convert streambuf to ostream
        allBranchesFile.rdbuf(&outbuf);     
