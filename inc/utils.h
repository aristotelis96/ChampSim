...
static OpType ConvertToOptype(uint8_t branch_type)
{
  if (branch_type==0){
    return OPTYPE_OP;
  }
  ...
  else if (branch_type==5){
    return OPTYPE_CALL_DIRECT_UNCOND;
  }
  else if (branch_type==6){
    return OPTYPE_CALL_INDIRECT_UNCOND;
  }
  else if (branch_type==7){
    return OPTYPE_CALL_DIRECT_COND;
  }
  else if (branch_type==8){
    return OPTYPE_CALL_INDIRECT_COND;
  }
  else if (branch_type==9){
    return OPTYPE_RET_UNCOND;
  }
  else if (branch_type==10){
    return OPTYPE_RET_COND;
  }
  else if (branch_type==11){
    return OPTYPE_MAX;
  }
}
#endif
